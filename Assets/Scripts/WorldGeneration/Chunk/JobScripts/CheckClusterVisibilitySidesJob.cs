...
[BurstCompile]
public struct CheckClusterVisibilitySidesJob : IJob
{
    ...
    [NativeDisableParallelForRestriction] public NativeArray<ClusterSidesDataStruct> ClusterSidesData;
    [ReadOnly] public int ClusterDataLength;
    public NativeList<ClusterSidesDataStruct>.ParallelWriter Writer;

    public void Execute()
    {
        for (int index = 0; index < ClusterDataLength; index++)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                ClusterSidesData[index * 6 + i] = data;
